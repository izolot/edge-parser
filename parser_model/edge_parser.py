...
class Parser(object):
    ...
    def search_by_time(self, start: str, end: str, uuid: str) -> dict:
        ...
        if camera_path is None:
            raise BaseException(
                "Error  -> Camera UUID is not found." +
                "Please add camera to archive_path/cameras.config")
        ...
        if start_time['hour'] != end_time['hour']:
            ...
            path = camera_path + os.sep + start_time[
                'day_path'] + os.sep + str(start_time['hour'])
    ...
    def parse_json_args(self, filename: str, json_dict: dict) -> dict:
        ...
        time = datetime.datetime.strptime(
            json_dict['time'].split('.')[0], "%Y-%m-%dT%H:%M:%S")
        ...
        args_dict['vehicle']['plate-text'] = vehicle[
            'license-plates'][0]['text']['ucode']
        return args_dict
    ...
    def create_excel(self, filename: str, data: dict) -> str:
        ...
        title_format = workbook.add_format(
            {'bg_color': '396cb4', 'align': 'center', 'valign': 'vcenter',
    ...
    def create_folder_excels(self, foldername, data, num_rows) -> str:
        ...
        for i in range(0, num_files):
            ...
            begin = num_rows * i
            self.create_excel(
                folder_path + "part_" + str(i + 1), data[begin:end])
